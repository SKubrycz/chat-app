import { BadRequestException, Inject, Injectable, NotFoundException, Res, Scope } from "@nestjs/common";
...
export class LoginService {
  ...
  async postLogin(res: Response, login: ILogin): Promise<Message> {
    ...
    if (!comparison) throw new BadRequestException("Invalid input data")
      ...
      res.cookie("access_token", accessToken, {expires: new Date(Date.now() + 6000000), httpOnly: true});
